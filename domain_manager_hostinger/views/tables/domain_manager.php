...
$where[] = "AND " . db_prefix() . "domain_manager.deleted = 0";
...
foreach ($rResult as $aRow) {
    ...
    $expiry = isset($aRow['expiry_date']) ? $aRow['expiry_date'] : null;
    
    if (empty($expiry) || $expiry == '0000-00-00' || $expiry == null) {
        $_data = '<span class="tw-text-neutral-400">N/A</span>';
    } else {
        try {
            $expiry_timestamp = strtotime($expiry);
            if ($expiry_timestamp === false) {
                $_data = '<span class="tw-text-neutral-400">Invalid Date</span>';
            } else {
                $expiry_formatted = date('M d, Y', $expiry_timestamp);
            ...
            $_data = '<span class="tw-text-neutral-400">Error: ' . $e->getMessage() . '</span>';
        }
    }
    $row[] = $_data;

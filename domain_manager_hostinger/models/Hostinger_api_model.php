...
class Hostinger_api_model extends App_Model
{
    ...
    public function sync_domains()
    {
        ...
        // Map domain field — Hostinger uses 'domain' key
            ...
            $domain_type = isset($domain['type']) ? $domain['type'] : 'external';
    ...
    public function sync_websites()
    {
        ...
        foreach ($websites as $site) {
            $site_domain = isset($site['domain']) ? strtolower(trim($site['domain'])) : null;
            ...
            // Check if website record already exists by hostinger_website_id
            $existing = null;
            if ($hostinger_id) {
